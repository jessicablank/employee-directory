...
const DataSort = () => {
    ...
    useEffect(() => {
        ...
        const sortFunctions = {
            ...
            username: (a,b) => {
                return a.login.username < b.login.username? -1 : a.login.username > b.login.username? 1 : 0;
            },
            password: (a,b) => {
                return a.login.password < b.login.password? -1 : a.login.password > b.login.password? 1 : 0;
            },
            age: (a,b) => {
        ...
        if(filterEmployees.length && sortObj){
            filterEmployees = filterEmployees.sort((a, b) => {
                ...
                sortedEmployees = sortObj.order === "dsc"? sortedEmployees*-1: sortedEmployees
